...
import { defaultCoursesContent, type CoursesContent } from '@/lib/contentDefaults';

const contentCollection = db?.collection('content');

export async function getCoursesContent(): Promise<CoursesContent & { error?: string }> {
  if (initError || !contentCollection) {
    return { ...defaultCoursesContent, error: initError };
  }
  
  try {
    const doc = await contentCollection.doc('courses').get();

    if (!doc.exists) {
      console.log('Courses content document not found, creating with default content.');
      await contentCollection.doc('courses').set(defaultCoursesContent);
      return defaultCoursesContent;
    }
    
    return doc.data() as CoursesContent;
  } catch (error: any) {
    console.error("Failed to fetch courses content from Firestore, falling back to default content. Error:", error);
    ...
    return { ...defaultCoursesContent, error: errorMessage };
  }
}
